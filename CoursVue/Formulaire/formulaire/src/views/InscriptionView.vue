...
export default {
  ...
  methods: {
    async Confirm () {
      ...
      data.append("username", this.Utilisateur.FirstName);
      data.append("password", this.Utilisateur.LastName);
      await axios.post('http://localhost/Cours/CoursPHP/cours/index.php', data).then(res => console.log(res))
    },
    ButtonSubmit(Value) {
      if (Value == true) {
